...
# paths -----------------
integrity_data <- "input/artf_data_for_dashboard/ESMW462021-June2023.xlsx" # please update the dataname as the one in your local directory
...
hawala_integrity <- readxl::read_excel(integrity_data, sheet = "MSINFEX", na = convert_to_na, guess_max = 100000)
...
source("R/comparative analysis/4_Hawala Transfer Fee.R") # NFI price and availability
...
writexl::write_xlsx(transfer_fee_domestic_intl_list, glue::glue("{output_path}Hawala Fee comparison_{date}.xlsx"), format_headers = T) # Hawala Fee
