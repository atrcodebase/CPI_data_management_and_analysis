...
## by week
terminated_emp_by_week_atr <- rbind(
  #terminated employees
  gov_emp_salary_atr %>% 
    group_by(week, month_name, Position_Terminated_Since_Political_Transition) %>% 
  ...
  gov_emp_salary_atr %>% 
    ...
    group_by(week, month_name, Alternative_sources) %>% 
  ...
  gov_emp_salary_atr %>% 
    ...
    group_by(week, month_name, Has_Alternative_sources) %>% 
  ...
  gov_emp_salary_atr %>% 
    filter(Position_Terminated_Since_Political_Transition == "Yes") %>%
    group_by(week, month_name, Salary_Paid) %>% 
  ...
  gov_emp_salary_atr %>% 
    ...
    group_by(week, month_name, Salary_Status) %>% 
...
terminated_emp_by_week_job_location_atr <- gov_emp_salary_atr %>% 
  group_by(week, month_name, Job_Location, Position_Terminated_Since_Political_Transition) %>% 
  summarize(Freq = length(Position_Terminated_Since_Political_Transition))

terminated_emp_list <- list(
  by_week = terminated_emp_by_week_atr,
  terminated_emp_job_location = terminated_emp_by_week_job_location_atr
)
