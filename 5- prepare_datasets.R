...
fi_sub_items <- fi_sub_items %>% mutate(
  ...
  PRICE_FI_CALCULATED = case_when(
    ...
    Item_In_Stock_Shop == "Yes" & choice == "Tomato paste" & Unit_FI == "Kilogram (KG)" ~ (Price_FI / Unit_Amount_FI),
    ...
    Item_In_Stock_Shop == "Yes" & choice == "Imported vegetable oil" & Unit_FI == "milliliter (mL)" ~ (Price_FI / Unit_Amount_FI) * 1000 * 1.084598698,
...
reshape_hawala <- function(data, transfer_fee_cols, dest_type, fee_type){
  ...
  for(i in 1:3){
    ...
    if(dest_type == "diff"){
      ...
      data <- data %>% filter(Transfer_Fee_Same %in% "Yes")
